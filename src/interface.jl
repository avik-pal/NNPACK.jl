...
function relu(x::AbstractArray{T,N}; inplace::Bool = true, nthreads = NNPACK_CPU_THREADS) where {T,N}
...
function ∇relu(x::AbstractArray{T,N}, dy::AbstractArray{T,N}; nthreads = NNPACK_CPU_THREADS) where {T,N}
...
function leakyrelu(x::AbstractArray{T,N}, negative_slope::AbstractFloat = 0.01; inplace::Bool = true, nthreads = NNPACK_CPU_THREADS) where {T,N}
    T == Float32 || error("NNPACK RELU supports only Float32")
    N != 1 || error("NNPACK RELU requires an array of 2 or more dimensions")
    nnp_relu_output(x, inplace ? x : similar(x), negative_slope = negative_slope, threadpool = pthreadpool_create(nthreads))
end

function ∇leakyrelu(x::AbstractArray{T,N}, dy::AbstractArray{T,N}, negative_slope::AbstractFloat = 0.0; nthreads = NNPACK_CPU_THREADS) where {T,N}
...
function softmax(x::AbstractVecOrMat{T}; nthreads = NNPACK_CPU_THREADS) where T
...
function softmax!(x::AbstractVecOrMat{T}; inplace::Bool = true, nthreads = NNPACK_CPU_THREADS) where T
    T == Float32 || error("NNPACK SOFTMAX supports only Float32")
    nnp_softmax_output(x, inplace ? x : similar(x), threadpool = pthreadpool_create(nthreads))
end

function softmax!(y::AbstractVecOrMat{T}, x::AbstractVecOrMat{T}; inplace::Bool = true, nthreads = NNPACK_CPU_THREADS) where T
    T == Float32 || error("NNPACK SOFTMAX supports only Float32")
    nnp_softmax_output(x, y, threadpool = pthreadpool_create(nthreads))
end

function maxpool(x::AbstractArray{T,4}, k::Tuple; pad = map(_->0,k), stride = k, nthreads = NNPACK_CPU_THREADS) where T
...
function fullyconnected(x::AbstractArray{T,2}, w::AbstractArray{T,2}; nthreads = NNPACK_CPU_THREADS) where T
...
function conv(x::AbstractArray{T,4}, w::AbstractArray{T,4}; pad = 0, stride = 1, dilation = 1, algo = 0, nthreads = NNPACK_CPU_THREADS) where T
    T == Float32 || error("NNPACK FULLY CONNECTED supports only Float32")
...
function conv(x::AbstractArray{T,4}, w::AbstractArray{T,4}, b::AbstractArray{T,1}; pad = 0, stride = 1, dilation = 1, algo = 0, nthreads = NNPACK_CPU_THREADS) where T
    T == Float32 || error("NNPACK FULLY CONNECTED supports only Float32")
    dilation == 1 || error("NNPACK does not support dilation > 1")
    pad_, stride_ = padtuple(x, pad), padtuple(x, stride)
